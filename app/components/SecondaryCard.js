        ...
        <div className="p-3 flex flex-wrap">
            <div className='w-full lg:w-1/4 flex gap-2 items-center justify-start'>
                <div className='w-full h-full'>
                    <Image src={course.img} alt='profileImg' width={350} height={350} className="min-h-[150px]"/>
                </div>
            </div>
            <div className="w-full lg:w-3/4">
                <div className="flex flex-col p-5">
                    <Link href={`/cursos/${course.slug}`}>
                        <p className='text-2xl text-special font-bold tracking-wide mb-3 cursor-pointer hover:text-blue-800'>{course.title}</p>
                    </Link>
                    <div className="flex gap-3 flex-wrap">
                        <p className="text-lg tracking-wide text-justify">{course.desc}</p>
                    </div>
                </div>
            </div>
        </div>
